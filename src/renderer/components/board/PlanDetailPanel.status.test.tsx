...
describe('PlanDetailPanel Schedule Controls', () => {
  beforeEach(() => {
    window.settingsApi = {
      getAgentTemplates: vi.fn().mockResolvedValue({
        ok: true,
  ...
  it('shows Pause button when schedule is running', async () => {
    const planContents = [
      '# Plan',
      '## Phase 1: Start',
      '- [ ] Task: Task A #schedule:interval,5m',
    ...
    const pauseBtn = screen.getByTestId('schedule-pause-btn')
    expect(pauseBtn).toBeInTheDocument()
    expect(pauseBtn).toHaveTextContent('Pause')
  })

  it('shows Resume button when schedule is paused', async () => {
    const planContents = [
      '# Plan',
      '## Phase 1: Start',
      '- [ ] Task: Task A #schedule:interval,5m',
    ...
    const resumeBtn = screen.getByTestId('schedule-resume-btn')
    expect(resumeBtn).toBeInTheDocument()
    expect(resumeBtn).toHaveTextContent('Resume')
  })

  it('shows Cancel button for active schedules', async () => {
    const planContents = [
      '# Plan',
      '## Phase 1: Start',
      '- [ ] Task: Task A #schedule:interval,5m',
    ...
    const cancelBtn = screen.getByTestId('schedule-cancel-btn')
  ...
  it('calls pause API when Pause button is clicked', async () => {
    ...
    expect(window.scheduleApi!.pause).toHaveBeenCalledWith({ scheduleId: 'schedule-1' })
  })
  ...
  it('calls resume API when Resume button is clicked', async () => {
    ...
    expect(window.scheduleApi!.resume).toHaveBeenCalledWith({ scheduleId: 'schedule-1' })
  })
  ...
  it('calls cancel API when Cancel button is clicked', async () => {
    ...
    expect(window.scheduleApi!.cancel).toHaveBeenCalledWith({ scheduleId: 'schedule-1' })
  })
})
